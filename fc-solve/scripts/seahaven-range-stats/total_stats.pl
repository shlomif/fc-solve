...
foreach my $is_solved ( 1, 0 )
{
    ...
    print "Num: "
        . $num . " ("
        . sprintf( "%.1f", ( $num / $total_num * 100 ) ) . "%)\n";
    print "Average Iters Num.: "
        . sprintf( "%.2f", ( $which->{'sum'} / $num ) ) . "\n";
    print "Median Iters Num.: "
        . $which->{'all_iters'}->[ int( $num / 2 ) ] . "\n";
    print "Std. Dev.: "
        . sprintf( "%.2f",
        sqrt( $which->{'sum_squares'} / $num - ( $which->{'sum'} / $num )**2 ) )
        . "\n";
