...
EMCC_LDFLAGS = --closure=1 -s WASM=$(WASM) -s TOTAL_MEMORY=$(EMCC_TOTAL_MEMORY) `cat $(NEEDED_FUNCTIONS_STR__FN)` -s EXPORTED_RUNTIME_METHODS=$(EMCC_EXPORTED_RUNTIME_METHODS) $(WASM_FLAGS) -s MODULARIZE=1 -s MALLOC=emmalloc $(EMCC_CFLAGS)
...
EMCC_POST_FLAGS := -s FILESYSTEM=0
