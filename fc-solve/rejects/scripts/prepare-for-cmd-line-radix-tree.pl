...
while ( my $line = <$module> )
{
        ...
        .. ( $line =~ m{/\* OPT-PARSE-END \*/} ) )
    {
        ...
        my $inner_strcmp_regex =
            qr{!strcmp *\( *argv\[arg\] *, *"-[a-zA-Z\-]+" *\) *};
        my $strcmp_regex = qr{\($inner_strcmp_regex\)|$inner_strcmp_regex};

        my $complete_strcmp =
            ( $line =~
m{\A\Q$ws_prefix\Eelse if \((?:$strcmp_regex *\|\| *)*$strcmp_regex\) *\n?\z}ms
            );
        ...
        if ( $complete_strcmp || $is_start_line )
        {
            ...
            $find_prefix .= join(
                ...
                map { $indent . $_ . "\n" } (
                    (
                        "else if ("
                            . join( "||",
                            map { qq{(!strcmp(argv[arg], "$_"))} } @strings )
                            . ")"
                    ),
                    "{",
                    "    opt = $opt;",
            ...
            $process_opts .= "${ws_prefix}case ${opt}: /* STRINGS="
                . join( "|", @strings ) . ";" . " */\n";
