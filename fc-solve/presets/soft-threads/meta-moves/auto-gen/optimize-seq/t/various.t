...
sub test_find_opt_command_output
{
    ...
    {
        # TEST:$cnt++;
        ok( !system( "mono", "find_opt.exe", $cmd, @$cmd_args, ),
            "$test_id ran successfully." );
    };

    # TEST:$cnt++;
    $trap->stderr_is( "", "$test_id did not return any errors." );

    # TEST:$cnt++;
    $trap->stdout_is( "$output\n", "$test_id output is OK.", );
...

{
    ...
    test_find_opt_command_output( "test_blacklist", [], "7\n8", );
}
...
sub test_scan_cmd_line
{
    ...
    return test_find_opt_command_output( "test_lookup_scan_cmd_line_by_id",
        [ $id, ], $cmd_line, );
}
...
sub test_lookup_iters
{
    ...
    return test_find_opt_command_output( "test_lookup_iters",
        [ $scan_id, $board_idx, ], $iters_num, );
}
...
sub test_process_sample_run
{
    ...
    return test_find_opt_command_output( "test_process_sample_run", [],
        $output, );
}
