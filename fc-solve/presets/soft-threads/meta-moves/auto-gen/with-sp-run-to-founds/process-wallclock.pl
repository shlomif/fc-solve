...
LINES_LOOP:
    ...
    foreach my $l_i ( 1 .. $#lines )
    {
        ...
        push @scan_recs,
            [ $new_iters - $last_iters, $new_time - $last_time, $status ];
    ...
    )
{
    ...
    my @unsolved_indexes = grep {
        !( $recs->[$_]->[$STATUS_IDX]
            && ( $recs->[$_]->[$ITERS_IDX] <= $iters_count ) );
    } keys(@$recs);
...
my %s_cmds =
    ( map { split( /\t/, $_ ) } ( io("scans.txt")->chomp->getlines() ) );
