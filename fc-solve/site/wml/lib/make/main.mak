...
$(STAMP1): $(LIBFREECELL_SOLVER_JS_DIR__DESTDIR_DATA)
	( cd $(LIBFREECELL_SOLVER_JS_DIR) && gmake -j4 -f $(FC_SOLVE_SOURCE_DIR)/../scripts/Makefile.to-javascript.mak SRC_DIR=$(FC_SOLVE_SOURCE_DIR) CMAKE_DIR=$(FC_SOLVE_CMAKE_DIR) ASMJS=0 && $(STRIP_TRAIL_SPACE) *.js *.html)
	touch $(STAMP1)

$(LIBFREECELL_SOLVER_ASMJS_JS): $(STAMP2)
$(LIBFREECELL_SOLVER_ASMJS_JS_DIR)/$(JS_MEM_BASE__ASMJS): $(STAMP2)

$(STAMP2): $(LIBFREECELL_SOLVER_JS_DIR__DESTDIR_DATA)
	( cd $(LIBFREECELL_SOLVER_ASMJS_JS_DIR) && gmake -j4 -f $(FC_SOLVE_SOURCE_DIR)/../scripts/Makefile.to-javascript.mak DATA_DESTDIR=$(LIBFREECELL_SOLVER_JS_DIR__DESTDIR) SRC_DIR=$(FC_SOLVE_SOURCE_DIR) CMAKE_DIR=$(FC_SOLVE_CMAKE_DIR) ASMJS=1 && touch $(JS_MEM_BASE__ASMJS) && $(STRIP_TRAIL_SPACE) *.js *.html)
	touch $@
...
clean:
	rm -f $(LIBFREECELL_SOLVER_JS_DIR)/*.{bc,js} $(STAMP1) $(STAMP2) $(TYPESCRIPT_DEST_FILES__NODE) $(TYPESCRIPT_DEST_FILES) $(TS_CHART_DEST)
