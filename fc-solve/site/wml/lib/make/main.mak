...
$(WASM_STAMP): $(LIBFREECELL_SOLVER_JS_DIR__DESTDIR_DATA)
	( cd $(LIBFREECELL_SOLVER_JS_DIR) && gmake -j4 -f $(FC_SOLVE_SOURCE_DIR)/../scripts/Makefile.to-javascript.mak RINUTILS_DIR="$(PWD)/lib/repos/rinutils-include" SRC_DIR=$(FC_SOLVE_SOURCE_DIR) CMAKE_DIR=$(FC_SOLVE_CMAKE_DIR) ASMJS=0 && $(STRIP_TRAIL_SPACE) *.js )
	touch $(WASM_STAMP)
...
$(ASMJS_STAMP): $(LIBFREECELL_SOLVER_JS_DIR__DESTDIR_DATA)
	( cd $(LIBFREECELL_SOLVER_ASMJS_JS_DIR) && gmake -j4 -f $(FC_SOLVE_SOURCE_DIR)/../scripts/Makefile.to-javascript.mak RINUTILS_DIR="$(PWD)/lib/repos/rinutils-include" DATA_DESTDIR=$(LIBFREECELL_SOLVER_JS_DIR__DESTDIR) SRC_DIR=$(FC_SOLVE_SOURCE_DIR) CMAKE_DIR=$(FC_SOLVE_CMAKE_DIR) ASMJS=1 && touch $(JS_MEM_BASE__ASMJS) && $(STRIP_TRAIL_SPACE) *.js )
	touch $@
