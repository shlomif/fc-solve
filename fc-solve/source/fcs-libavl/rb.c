...
static inline void rb_set_link(
    struct rb_node *node, int myindex, struct rb_node *val)
{
...
static inline void rb_set_link(
    struct rb_node *node, int myindex, struct rb_node *val)
{
    node->rb_mylink[myindex] =
        (((uintptr_t)val) | (node->rb_mylink[myindex] & 0x1));
}

static inline enum rb_color rb_set_color(
    struct rb_node *const node, const enum rb_color color)
{
...
struct rb_table *rb_create(rb_comparison_func *compare, void *param GCC_UNUSED,
    meta_allocator *meta_alloc, void **common_recycle_bin)
{
    ...
    tree->rb_proto_root.rb_mylink[0] = 0;
    tree->rb_proto_root.rb_mylink[1] = 0;
...
avl_key_type *rb_probe(struct rb_table *tree, void *item)
{
    ...
    {
        n = fcs_compact_alloc_ptr(&(tree->dict_allocator), sizeof(*n));
    }
...
void *rb_delete(struct rb_table *tree, const void *item)
{
    ...
    for (cmp = -1; cmp != 0; cmp = tree->rb_compare(item, NODE_DATA_PTR(p)
#ifdef AVL_with_rb_param
    ...
    if (rb_get_color(p) == RB_BLACK)
    {
        ...
        {
            struct rb_node *x =
                rb_process_link(pa[k - 1]->rb_mylink[da[k - 1]]);
            ...
            if (da[k - 1] == 0)
            {
                ...
                if ((rb_process_link(w->rb_mylink[0]) == NULL ||
                        rb_get_color(rb_process_link(w->rb_mylink[0])) ==
                            RB_BLACK) &&
                    (rb_process_link(w->rb_mylink[1]) == NULL ||
                        rb_get_color(rb_process_link(w->rb_mylink[1])) ==
                            RB_BLACK))
...
void *__attribute__((pure)) rb_t_cur(struct rb_traverser *trav)
{
    assert(trav != NULL);
