 ...
 * */
    ...
    if (!STRUCT_QUERY_FLAG(instance, FCS_RUNTIME_OPT_TESTS_ORDER_WAS_SET))
    {
        ...
        // total_move_funcs_bitmask to a valid tests order.
             ...
             bit_idx++, total_move_funcs_bitmask >>= 1)
        {
            ...
            {
                move_funcs[num_move_funcs++] = (fcs_moves_group){
                    .shuffling_type = FCS_SINGLE, .m.idx = bit_idx};
            }
        }
        ...
        instance->opt_moves = (typeof(instance->opt_moves)){
            ...
            .shuffling_type = FCS_NO_SHUFFLING,
            .m.move_funcs =
                SMALLOC(instance->opt_moves.m.move_funcs, MOVES_GROW_BY),
        };
        instance->opt_moves.m.move_funcs[0] =
            (typeof(instance->opt_moves.m.move_funcs[0])){
                .m.move_funcs = move_funcs,
                .num = num_move_funcs,
                .shuffling_type = FCS_NO_SHUFFLING,
            };
...
void fc_solve__moves_order__call(const fcs_moves_group g,
    fcs_soft_thread *const soft_thread, fcs_kv_state pass,
    fcs_derived_states_list *const derived_list)
{
...
#define the_soft_dfs_info (&(DFS_VAR(soft_thread, soft_dfs_info)[DEPTH()]))
        ...
        ++DEPTH();
        fcs_derived_states_list prev_derived_list = {
            .num_states = 0,
            .states = NULL,
        };
        fcs_derived_states_list next_derived_list = {
            .num_states = 0,
            .states = NULL,
        };
             ...
             i++)
        {
            ...
            next_derived_list = (fcs_derived_states_list){
                .num_states = 0,
                .states = NULL,
            };
        }
...
#undef DEPTH
    ...
    rating_with_index rand_array[num_states];
...
static inline int dfs_solve(fcs_soft_thread *const soft_thread)
{
    ...
    fcs_moves_group the_moves_list;
...
#define DEPTH_MARGIN 15
        ...
        if (the_soft_dfs_info->current_state_index == derived_list.num_states)
        {
            ...
            const fcs_state_weighting *const weighting =
                &(THE_MOVE_FUNCS_LIST.m.move_funcs[orig_idx].weighting);
    ...
    const char *const moves_order FCS__PASS_ERR_STR(char **const error_string))
{
    ...
    if (depth_idx == soft_thread->by_depth_moves_order.num)
    {
        ...
        soft_thread->by_depth_moves_order.by_depth_moves[depth_idx]
            .moves_order = (fcs_moves_group){
            .num = 0, .m.move_funcs = NULL, .shuffling_type = FCS_NO_SHUFFLING};
    }
